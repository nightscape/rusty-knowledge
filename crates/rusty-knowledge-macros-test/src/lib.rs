...
mod tests {
    ...
    fn test_require_precondition_extraction() {
        ...
        assert!(delete_op.precondition.is_some(), "delete operation should have precondition");

        // Find set_flag operation
        let set_flag_op = ops.iter().find(|op| op.name == "set_flag").unwrap();
        assert!(set_flag_op.precondition.is_some(), "set_flag operation should have precondition");
        ...
        assert!(set_priority_op.precondition.is_some(), "set_priority operation should have precondition");

        // Find no_precondition operation
        let no_precondition_op = ops.iter().find(|op| op.name == "no_precondition").unwrap();
        assert!(no_precondition_op.precondition.is_none(), "no_precondition operation should not have precondition");
    }
    ...
    fn test_precondition_closure_evaluation() {
        ...
        params_valid.insert("id".to_string(), Box::new(Value::String("test123".to_string())));
        ...
        assert!(result_valid.is_ok(), "Precondition should pass for valid input");
        assert_eq!(result_valid.unwrap(), true, "Precondition should return true");
        ...
        assert_eq!(result_invalid.unwrap(), false, "Precondition should return false for empty string");
    }
    ...
    fn test_precondition_with_multiple_requires() {
        ...
        let mut params_valid: HashMap<String, Box<dyn Any + Send + Sync>> = HashMap::new();
        params_valid.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        params_valid.insert("priority".to_string(), Box::new(Value::Integer(3)));

        let result = precondition(&params_valid);
        assert!(result.is_ok(), "Precondition should pass for valid priority");
        assert_eq!(result.unwrap(), true, "Precondition should return true for priority 3");

        // Test invalid priority (too low)
        let mut params_low: HashMap<String, Box<dyn Any + Send + Sync>> = HashMap::new();
        params_low.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        ...
        assert_eq!(result_low.unwrap(), false, "Precondition should return false for priority 0");

        // Test invalid priority (too high)
        let mut params_high: HashMap<String, Box<dyn Any + Send + Sync>> = HashMap::new();
        params_high.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        ...
        assert_eq!(result_high.unwrap(), false, "Precondition should return false for priority 6");
    }
    ...
    fn test_precondition_with_bool_parameter() {
        ...
        params_true.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        ...
        assert_eq!(result_true.unwrap(), true, "Precondition should return true");

        // Test with false value
        let mut params_false: HashMap<String, Box<dyn Any + Send + Sync>> = HashMap::new();
        params_false.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        ...
        assert_eq!(result_false.unwrap(), true, "Precondition should return true for false (it's a valid bool)");
    }
    ...
    fn test_precondition_missing_parameter() {
        ...
        assert!(result.is_err(), "Precondition should error when required parameter is missing");
        assert!(result.unwrap_err().contains("Missing"), "Error should mention missing parameter");
    }
    ...
    fn test_precondition_boundary_values() {
        ...
        params_min.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        params_min.insert("priority".to_string(), Box::new(Value::Integer(1)));

        let result_min = precondition(&params_min);
        assert!(result_min.is_ok(), "Precondition should pass for priority 1 (lower bound)");
        assert_eq!(result_min.unwrap(), true, "Precondition should return true for priority 1");

        let mut params_max: HashMap<String, Box<dyn Any + Send + Sync>> = HashMap::new();
        params_max.insert("id".to_string(), Box::new(Value::String("test".to_string())));
        params_max.insert("priority".to_string(), Box::new(Value::Integer(5)));

        let result_max = precondition(&params_max);
        assert!(result_max.is_ok(), "Precondition should pass for priority 5 (upper bound)");
        assert_eq!(result_max.unwrap(), true, "Precondition should return true for priority 5");
    }
}
