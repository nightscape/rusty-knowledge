use super::models::{
    CommandResponse, CreateTaskRequest, SyncCommand, SyncResponse,
...
impl TodoistClient {
    ...
    fn format_reqwest_error(e: reqwest::Error, url: &str, operation: &str) -> String {
        ...
        if e.is_timeout() {
            format!("Failed to {} for {}: timeout - request took too long (check network or increase timeout)", operation, url)
        } else if e.is_connect() {
            format!("Failed to {} for {}: connection error - check network connectivity, DNS resolution, and firewall settings. Error: {}", operation, url, e)
        } else if e.is_request() {
            format!("Failed to {} for {}: request error - invalid URL format or malformed request parameters. Error: {}", operation, url, e)
        } else if e.is_decode() {
            format!("Failed to {} for {}: decode error - unexpected response format from server. Error: {}", operation, url, e)
        } else {
            ...
            if display_str.contains("error sending request") {
                format!("Failed to {} for {}: network/connection issue - check internet connection, API availability, and proxy settings. Debug details: {}", operation, url, error_str)
            } else if display_str.contains("certificate") || display_str.contains("TLS") || error_str.contains("certificate") || error_str.contains("TLS") {
                format!("Failed to {} for {}: TLS/certificate error - check SSL certificate configuration. Error: {}", operation, url, e)
            } else if display_str.contains("redirect") || error_str.contains("redirect") {
                format!("Failed to {} for {}: redirect error - too many redirects or invalid redirect. Error: {}", operation, url, e)
            } else {
                format!("Failed to {} for {}: {}. Debug details: {}", operation, url, display_str, error_str)
            }
        }
    }
                            ...
                            if let Some(map) = sync_status.as_object() {
                                ...
                                for (uuid, value) in map {
                                    ...
                                    if let Some(status_str) = value.as_str() {
                                        ...
                                        match serde_json::from_value::<CommandResponse>(value.clone()) {
                                            Ok(mut resp) => {
                                                ...
                                                if resp.temp_id_mapping.is_none() {
                                                    resp.temp_id_mapping = top_level_temp_id_mapping.clone();
                                                }
                                                responses.push(resp);
                                            }
                                            Err(_) => {
                                                ...
                                                let error = obj.get("error")
                                                    .and_then(|v| v.as_str())
                                                    .map(|s| s.to_string());
                                                let temp_id_mapping = obj.get("temp_id_mapping")
                                                    .cloned()
                                        ...
                                        return Err(format!("Unexpected sync_status value type for UUID {}: {}", uuid, value).into());
                                    }
                                }
                                Ok(responses)
                            } else {
                                Err(format!("sync_status is not an array or object: {}", response_text).into())
                            }
                        }
                    }
                } else {
    ...
    async fn execute_command(&self, command: SyncCommand) -> Result<CommandResponse> {
        ...
        debug!("[TodoistClient] Executing command: type={}, uuid={}", command.command_type, command_uuid);
        ...
        debug!("[TodoistClient] Command response received: uuid={}, response_length={}", command_uuid, response_text.len());

        let cmd_responses = Self::parse_command_response(&response_text)
            .map_err(|e| {
                error!("[TodoistClient] Failed to parse command response: {} - Response: {}", e, &response_text.chars().take(200).collect::<String>());
                e
            })?;
        ...
        if cmd_result.status != "ok" {
            let error_msg = cmd_result.error.unwrap_or_else(|| "Unknown error".to_string());
    ...
    pub async fn sync_items(&self, sync_token: Option<&str>) -> Result<SyncResponse> {
        ...
        info!("[TodoistClient] Syncing items: sync_token={}", if sync_token == "*" { "full_sync" } else { "incremental" });
        ...
        debug!("[TodoistClient] Sync response received: length={}", response_text.len());
        ...
        if let Ok(error_resp) = serde_json::from_str::<serde_json::Value>(&response_text) {
            if error_resp.get("error").is_some() {
                let error = format!(
                    "Todoist API error: {} - {}",
                    error_resp.get("error_tag").and_then(|v| v.as_str()).unwrap_or("UNKNOWN"),
                    error_resp.get("error").and_then(|v| v.as_str()).unwrap_or("Unknown error")
                );
                error!("[TodoistClient] {}", error);
        ...
        let response: SyncResponse = serde_json::from_str(&response_text)
            .map_err(|e| {
                let error = format!("Failed to parse SyncResponse: {} - Response (first 500): {}", e, &response_text.chars().take(500).collect::<String>());
                error!("[TodoistClient] {}", error);
                error
            })?;

        info!("[TodoistClient] Sync completed: items_count={}, sync_token_present={}",
                  response.items.len(),
