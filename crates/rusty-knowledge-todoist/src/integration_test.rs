use rusty_knowledge::core::predicate::Eq;
...
use rusty_knowledge::core::traits::Queryable;
use rusty_knowledge::storage::types::Value;
use crate::{TodoistTaskDataSource, TodoistTask};
...
mod tests {
    ...
    async fn test_query_high_priority_tasks() {
        ...
        use rusty_knowledge::core::predicate::Gt;
    ...
    async fn test_create_and_delete_task() {
        ...
        fields.insert("content".to_string(), Value::String(new_task.content.clone()));
        fields.insert("project_id".to_string(), Value::String(new_task.project_id.clone()));
        let task_id = <QueryableCache<_, TodoistTask> as CrudOperationProvider<TodoistTask>>::create(&cache, fields).await.expect("Failed to create task");
        println!("Created task with ID: {}", task_id);

        let created_task = <QueryableCache<_, TodoistTask> as DataSource<TodoistTask>>::get_by_id(&cache, &task_id)
            .await
            .expect("Failed to fetch created task")
            .expect("Task not found");
        println!("Retrieved task: {}", created_task.content);

        <QueryableCache<_, TodoistTask> as CrudOperationProvider<TodoistTask>>::delete(&cache, &task_id).await.expect("Failed to delete task");
    ...
    async fn test_update_task() {
        ...
        let mut task_to_update = <QueryableCache<_, TodoistTask> as DataSource<TodoistTask>>::get_by_id(&cache, &task_id)
            .await
        ...
        <QueryableCache<_, TodoistTask> as CrudOperationProvider<TodoistTask>>::set_field(&cache, &task_id, "content", Value::String(task_to_update.content.clone())).await.expect("Failed to update content");
        <QueryableCache<_, TodoistTask> as CrudOperationProvider<TodoistTask>>::set_field(&cache, &task_id, "priority", Value::Integer(task_to_update.priority as i64)).await.expect("Failed to update priority");
        <QueryableCache<_, TodoistTask> as CrudOperationProvider<TodoistTask>>::set_field(&cache, &task_id, "description", Value::String(task_to_update.description.clone().unwrap_or_default())).await.expect("Failed to update description");

        let updated_task = <QueryableCache<_, TodoistTask> as DataSource<TodoistTask>>::get_by_id(&cache, &task_id)
            .await
