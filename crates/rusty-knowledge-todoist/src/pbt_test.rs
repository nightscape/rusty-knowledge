...
use rusty_knowledge::core::datasource::{DataSource, CrudOperationProvider};
...
pub enum TodoistTransition {
    ...
    CreateItem { task_id: String, project_id: String, content: String },
    /// Update a task's content or priority
    UpdateItem { task_id: String, content: Option<String>, priority: Option<i32> },
    /// Delete a task
...
fn apply_to_reference(state: &mut ReferenceState, transition: &TodoistTransition) -> (Option<Vec<TodoistTask>>, Option<String>) {
    match transition {
        ...
        TodoistTransition::CreateItem { task_id, project_id, content } => {
        ...
        TodoistTransition::UpdateItem { task_id, content, priority } => {
...
async fn apply_to_todoist(
    ...
    match transition {
        TodoistTransition::CreateProject { project_id: _expected_project_id, name } => {
            // Create project via client
            let actual_project_id = test.provider.client.create_project(&name).await
                .map_err(|e| e.to_string())?;
            Ok((None, Some(actual_project_id)))
        }
        TodoistTransition::CreateItem { task_id: _expected_task_id, project_id, content } => {
            ...
            let actual_project_id = {
                ...
                mapping.get(project_id).cloned().unwrap_or_else(|| {
                    eprintln!("[PBT] Warning: No mapping found for project {}, using as-is", project_id);
                    project_id.clone()
                })
            };

            // Create task using new API
            use rusty_knowledge::core::datasource::CrudOperationProvider;
            let mut fields = HashMap::new();
            fields.insert("content".to_string(), Value::String(content.clone()));
            fields.insert("project_id".to_string(), Value::String(actual_project_id.clone()));
            let _actual_task_id = <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::create(&datasource, fields).await
                .map_err(|e| e.to_string())?;
        ...
        TodoistTransition::UpdateItem { task_id, content, priority } => {
            let current_task = <TodoistTaskDataSource as DataSource<TodoistTask>>::get_by_id(&datasource, task_id).await
                .map_err(|e| e.to_string())?
            ...
            use rusty_knowledge::core::datasource::CrudOperationProvider;
            if let Some(c) = content {
                <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::set_field(&datasource, task_id, "content", Value::String(c.clone())).await
                    .map_err(|e| e.to_string())?;
            }
            if let Some(p) = priority {
                <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::set_field(&datasource, task_id, "priority", Value::Integer(*p as i64)).await
                    .map_err(|e| e.to_string())?;
            }
            Ok((None, None))
        }
        TodoistTransition::DeleteItem { task_id } => {
            use rusty_knowledge::core::datasource::CrudOperationProvider;
            <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::delete(&datasource, task_id).await
                .map_err(|e| e.to_string())?;
            Ok((None, None))
        }
        TodoistTransition::CompleteItem { task_id } => {
            use rusty_knowledge::core::datasource::CrudOperationProvider;
            <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::set_field(&datasource, task_id, "completed", Value::Boolean(true)).await
                .map_err(|e| e.to_string())?;
            Ok((None, None))
        }
        TodoistTransition::UncompleteItem { task_id } => {
            use rusty_knowledge::core::datasource::CrudOperationProvider;
            <TodoistTaskDataSource as CrudOperationProvider<TodoistTask>>::set_field(&datasource, task_id, "completed", Value::Boolean(false)).await
                .map_err(|e| e.to_string())?;
            Ok((None, None))
        }
        TodoistTransition::FullSync => {
...
fn verify_states_match(
    ...
    let test_project_ids: std::collections::HashSet<String> = reference.projects.keys().cloned().collect();
    if test_project_ids.is_empty() {
        eprintln!("[PBT] verify_states_match: no test projects, skipping verification");
        return;
    }

    eprintln!("[PBT] verify_states_match: verifying {} projects, {} tasks",
             test_project_ids.len(), reference.tasks.len());

    // Get all tasks from TodoistTaskDataSource
    let todoist_tasks = tokio::task::block_in_place(|| {
        handle.block_on(todoist.get_all())
    })
    .expect("Failed to get all tasks from TodoistTaskDataSource");

    eprintln!("[PBT] verify_states_match: retrieved {} tasks from Todoist", todoist_tasks.len());
    ...
    let reference_test_tasks: HashMap<String, TodoistTask> = reference.tasks
        .iter()
...
fn generate_transitions(state: &ReferenceState) -> BoxedStrategy<TodoistTransition> {
    ...
    let create_item = (
        ...
        .prop_map(|(project_id, content, task_id)| TodoistTransition::CreateItem {
    ...
    let update_item = (
        ...
        .prop_map(|(task_id, content, priority)| TodoistTransition::UpdateItem {
            task_id,
            content,
            priority,
        })
    ...
    let uncomplete_item = task_ids.iter()
        .filter(|id| {
            state.tasks.get(*id)
                .map(|t| t.completed)
                .unwrap_or(false)
    ...
    let uncomplete = if uncomplete_item.is_empty() {
        ...
        Some(prop::sample::select(uncomplete_item)
...
impl StateMachineTest for TodoistTest {
    ...
    fn apply(
        ...
        let (ref_result, _expected_project_id) = apply_to_reference(&mut ref_state_clone, &transition);
        ...
        if let (TodoistTransition::CreateProject { project_id: expected_id, .. }, Some(actual_id)) = (&transition, actual_project_id) {
            let mut mapping = state.project_id_mapping.lock().unwrap();
            mapping.insert(expected_id.clone(), actual_id.clone());
            eprintln!("[PBT] Stored project ID mapping: {} -> {}", expected_id, actual_id);
        }

        // For sync operations, compare results
        if let (Some(ref_tasks), Some(todoist_tasks)) = (ref_result, todoist_result) {
            // Build maps for comparison
            let ref_map: HashMap<String, TodoistTask> = ref_tasks
                .into_iter()
                .map(|t| (t.id.clone(), t))
                .collect();
            ...
            let test_project_ids: std::collections::HashSet<String> = ref_state.projects.keys().cloned().collect();

            let ref_test_count = ref_map.values()
                .filter(|t| test_project_ids.contains(&t.project_id))
                .count();

            let todoist_test_count = todoist_map.values()
                .filter(|t| test_project_ids.contains(&t.project_id))
                .count();
    ...
    fn check_invariants(
        ...
        eprintln!("[PBT] check_invariants called: projects={}, tasks={}",
                 ref_state.projects.len(), ref_state.tasks.len());
    ...
    fn teardown(
        ...
        eprintln!("[PBT] teardown called: cleaning up {} projects", ref_state.projects.len());
        ...
        if let Some(project_ids) = projects_to_delete {
            for project_id in project_ids {
                ...
                let result = tokio::task::block_in_place(|| {
                    handle.block_on(async {
                        provider_arc.delete_project(&project_id).await
                    })
                });

                if let Err(e) = result {
                    eprintln!("[PBT] Warning: Failed to delete project {}: {}", project_id, e);
                } else {
