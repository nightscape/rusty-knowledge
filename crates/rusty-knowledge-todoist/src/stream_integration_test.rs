...
mod tests {
    ...
    use rusty_knowledge::core::datasource::{Change, DataSource, CrudOperationProvider};
    ...
    impl DataSource<TodoistTask> for InMemoryDataSource {
        async fn get_all(&self) -> rusty_knowledge::core::datasource::Result<Vec<TodoistTask>> {
            Ok(self.tasks.read().await.values().cloned().collect())
        }

        async fn get_by_id(&self, id: &str) -> rusty_knowledge::core::datasource::Result<Option<TodoistTask>> {
    ...
    async fn create_fake_with_cache() -> (Arc<TodoistTaskFake>, Arc<InMemoryDataSource>) {
        ...
        let fake = Arc::new(TodoistTaskFake::new(Arc::clone(&cache) as Arc<dyn DataSource<TodoistTask>>));
    ...
    async fn test_fake_datasource_stream_emission() {
        ...
        fields.insert("content".to_string(), Value::String("Test Task".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
        ...
        match &changes[0] {
            ...
            Change::Updated { .. } | Change::Deleted { .. } => panic!("Expected Created, got {:?}", changes[0]),
        }
    }
    ...
    async fn test_cache_reads_from_local_db() {
        ...
        let mut fields = HashMap::new();
        fields.insert("content".to_string(), Value::String("Cache Test Task".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
    ...
    async fn test_cache_delegates_writes() {
        ...
        let mut fields = HashMap::new();
        fields.insert("content".to_string(), Value::String("Write Test Task".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
    ...
    async fn test_cache_updates_via_stream() {
        ...
        // Create a task
        let mut fields = HashMap::new();
        fields.insert("content".to_string(), Value::String("Original Content".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
    ...
    async fn test_multiple_caches_same_stream() {
        ...
        // Create a task
        let mut fields = HashMap::new();
        fields.insert("content".to_string(), Value::String("Multi Cache Task".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
    ...
    async fn test_delete_via_stream() {
        ...
        // Create a task
        let mut fields = HashMap::new();
        fields.insert("content".to_string(), Value::String("To Delete".to_string()));
        fields.insert("project_id".to_string(), Value::String("project-123".to_string()));
