...
use super::types::{ApiError, Block, NewBlock};
...
impl Default for ReferenceState {
    fn default() -> Self {
        ...
        match tokio::runtime::Handle::try_current() {
            Ok(handle) => {
                // Use existing runtime (Flutter/FRB context)
...
pub fn translate_id(mem_id: &str, id_map: &HashMap<String, String>) -> Option<String> {
    if mem_id == super::types::ROOT_PARENT_ID {
...
pub async fn apply_transition<R: CoreOperations>(
    ...
    match transition {
        BlockTransition::CreateBlock { parent_id, content } => {
            let block = backend
                .create_block(parent_id.clone(), content.clone(), None)
                .await?;
            Ok(vec![block])
        }
        BlockTransition::UpdateBlock { id, content } => {
            backend.update_block(id, content.clone()).await?;
            Ok(vec![])
        }
