//! Tests for MutableBlockDataSource trait methods
//!
...
#[cfg(test)]
mod tests {
    use crate::core::datasource::{BlockEntity, DataSource, CrudOperationProvider, MutableBlockDataSource, Result};
    ...
    impl DataSource<TestBlock> for TestBlockDataSource {
        async fn get_all(&self) -> Result<Vec<TestBlock>> {
            let backend = self.backend.read().await;
            let conn = backend.get_connection()?;
            let mut stmt = conn.prepare("SELECT id, parent_id, depth, sort_key, content FROM blocks").await?;
    ...
    #[async_trait]
    impl CrudOperationProvider<TestBlock> for TestBlockDataSource {
        async fn set_field(&self, id: &str, field: &str, value: Value) -> Result<()> {
            ...
            StorageBackend::update(&mut *backend, "blocks", id, updates).await
                .map_err(|e: crate::storage::StorageError| -> Box<dyn std::error::Error + Send + Sync> {
        ...
        async fn create(&self, fields: HashMap<String, Value>) -> Result<String> {
            ...
            StorageBackend::insert(&mut *backend, "blocks", entity).await
                .map_err(|e: crate::storage::StorageError| -> Box<dyn std::error::Error + Send + Sync> {
        ...
        async fn delete(&self, id: &str) -> Result<()> {
            let mut backend = self.backend.write().await;
            StorageBackend::delete(&mut *backend, "blocks", id).await
                .map_err(|e: crate::storage::StorageError| -> Box<dyn std::error::Error + Send + Sync> {
    ...
    async fn insert_block(
        ...
        fields.insert("content".to_string(), Value::String(format!("Content {}", id)));
    ...
    async fn test_indent_block() {
        ...
        ds.indent_block("B", "A").await.unwrap();
    ...
    async fn test_outdent_block() {
        ...
        ds.outdent_block("C").await.unwrap();
    ...
    async fn test_outdent_at_root_fails() {
        ...
        let result = ds.outdent_block("A").await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Cannot outdent root block"));
    }
    ...
    async fn test_split_block() {
        ...
        fields.insert("sort_key".to_string(), Value::String(gen_key_between(None, None).unwrap()));
