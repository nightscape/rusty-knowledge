...
impl<'a> RowView<'a> {
    ...
    pub fn is_completed(&self) -> Result<bool> {
        match self.data.get("completed") {
            ...
            Some(v) => v
                .as_bool()
                .context("Field 'completed' is not a boolean"),
        }
    }
...
mod tests {
    ...
    fn create_test_entity() -> StorageEntity {
        ...
        entity.insert("parent_id".to_string(), Value::String("block-0".to_string()));
        entity.insert("depth".to_string(), Value::Integer(2));
        entity.insert("sort_key".to_string(), Value::String("a0".to_string()));
        entity.insert("content".to_string(), Value::String("Test content".to_string()));
