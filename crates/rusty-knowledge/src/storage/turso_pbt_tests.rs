...
use super::{TursoBackend, RowChange, ChangeData};
use crate::api::streaming::ChangeOrigin;
...
pub enum StorageTransition {
    ...
    Insert { entity: String, id: String, value: String },
    Update { entity: String, id: String, value: String },
    Delete { entity: String, id: String },
    Query { entity: String, filter: Filter },
    Get { entity: String, id: String },
    SetVersion { entity: String, id: String, version: String },
    EnableCDC,
    CreateMaterializedView { view_name: String, entity: String },
    CreateViewStream { view_name: String },
}
...
impl StorageTest {
    ...
    pub fn new(handle: &tokio::runtime::Handle) -> Self {
        let backend = tokio::task::block_in_place(|| {
            handle.block_on(TursoBackend::new_in_memory())
        })
        .unwrap();
        Self {
            backend,
    ...
    pub fn new_with_file(handle: &tokio::runtime::Handle, db_path: &str) -> Self {
        let backend = tokio::task::block_in_place(|| {
            handle.block_on(TursoBackend::new(db_path))
        })
        .unwrap();
        Self {
            backend,
...
fn apply_filter_ref(entity: &StorageEntity, filter: &Filter) -> bool {
    match filter {
        ...
        Filter::In(field, values) => {
            entity.get(field).map(|v| values.contains(v)).unwrap_or(false)
        }
        Filter::And(filters) => {
            filters.iter().all(|f| apply_filter_ref(entity, f))
        }
        Filter::Or(filters) => {
            filters.iter().any(|f| apply_filter_ref(entity, f))
        }
        Filter::IsNull(field) => {
            entity.get(field).map(|v| matches!(v, Value::Null)).unwrap_or(true)
        }
        Filter::IsNotNull(field) => {
            entity.get(field).map(|v| !matches!(v, Value::Null)).unwrap_or(false)
        }
    }
}
...
fn apply_to_reference(state: &mut ReferenceState, transition: &StorageTransition) -> Option<Vec<StorageEntity>> {
    match transition {
        ...
        StorageTransition::Insert { entity, id, value } => {
            ...
            state.entities.get_mut(entity).unwrap().insert(id.clone(), data.clone());
            state.versions.get_mut(entity).unwrap().insert(id.clone(), None);

            // Track CDC event if enabled
            if state.cdc_enabled {
                state.cdc_events.push((entity.clone(), "INSERT".to_string(), id.clone()));
            }
            ...
            for (view_name, (view_entity, _count)) in &state.materialized_views {
                if view_entity == entity {
                    if let Some(changes_vec) = state.view_stream_changes.get(view_name) {
                        ...
                        state.view_rowids.entry(view_name.clone()).or_default().insert(id.clone(), rowid);
        ...
        StorageTransition::Update { entity, id, value } => {
            ...
            // Track CDC event if enabled
            if state.cdc_enabled {
                state.cdc_events.push((entity.clone(), "UPDATE".to_string(), id.clone()));
            }
            ...
            for (view_name, (view_entity, _count)) in &state.materialized_views {
                if view_entity == entity {
                    if let Some(changes_vec) = state.view_stream_changes.get(view_name) {
                        ...
                        let rowid = state.view_rowids.get(view_name)
                        ...
                        updated_data_with_rowid.insert("_rowid".to_string(), Value::String(rowid.to_string()));
                        let change = RowChange {
                            relation_name: view_name.clone(),
        ...
        StorageTransition::Delete { entity, id } => {
            ...
            // Track CDC event if enabled
            if state.cdc_enabled {
                state.cdc_events.push((entity.clone(), "DELETE".to_string(), id.clone()));
            }
        ...
        StorageTransition::SetVersion { entity, id, version } => {
            state.versions.get_mut(entity).unwrap().insert(id.clone(), Some(version.clone()));
        ...
        StorageTransition::CreateMaterializedView { view_name, entity } => {
            ...
            state.materialized_views.insert(view_name.clone(), (entity.clone(), count));
        ...
        StorageTransition::CreateViewStream { view_name } => {
            ...
            state.view_stream_changes.insert(view_name.clone(), Arc::new(Mutex::new(Vec::new())));
...
/// Returns the result (for Query/Get operations that return data)
async fn apply_to_turso(test: &mut StorageTest, transition: &StorageTransition, handle: &tokio::runtime::Handle) -> Result<Option<Vec<StorageEntity>>, String> {
    ...
    match transition {
        ...
        StorageTransition::Insert { entity, id, value } => {
            ...
            if let Some(conn) = &test.cdc_connection {
                ...
                let values: Vec<_> = data.values().map(|v| backend.value_to_sql_param(v)).collect();
                ...
                backend.insert(entity, data).await
                    .map_err(|e| e.to_string())?;
            }
            Ok(None)
        }
        StorageTransition::Update { entity, id, value } => {
            ...
            if let Some(conn) = &test.cdc_connection {
                ...
                backend.update(entity, id, data).await
                    .map_err(|e| e.to_string())?;
            }
            Ok(None)
        }
        StorageTransition::Delete { entity, id } => {
            ...
            if let Some(conn) = &test.cdc_connection {
                ...
                backend.delete(entity, id).await
                    .map_err(|e| e.to_string())?;
            }
            Ok(None)
        }
        StorageTransition::Query { entity, filter } => {
            let results = backend.query(entity, filter.clone()).await
                .map_err(|e| e.to_string())?;
            Ok(Some(results))
        }
        StorageTransition::Get { entity, id } => {
            let result = backend.get(entity, id).await
                .map_err(|e| e.to_string())?;
            Ok(Some(result.into_iter().collect()))
        }
        StorageTransition::SetVersion { entity, id, version } => {
            backend.set_version(entity, id, version.clone()).await
                .map_err(|e| e.to_string())?;
            Ok(None)
        }
        StorageTransition::EnableCDC => {
            // Create and store CDC-enabled connection
        ...
        StorageTransition::CreateViewStream { view_name } => {
            ...
            test.view_stream_changes.insert(view_name.clone(), changes.clone());
            ...
            test.view_stream_handles.insert(view_name.clone(), handle_inner);
            Ok(None)
        }
    }
}
...
fn check_preconditions(state: &ReferenceState, transition: &StorageTransition) -> bool {
    match transition {
        ...
        StorageTransition::CreateViewStream { view_name } => {
            ...
            state.materialized_views.contains_key(view_name) && !state.view_stream_changes.contains_key(view_name)
        }
    }
}
...
fn verify_states_match(
    ...
    for (entity_name, ref_entities) in &reference.entities {
        ...
        for (id, ref_data) in ref_entities {
            let turso_data = tokio::task::block_in_place(|| {
                handle.block_on(turso.get(entity_name, id))
            })
            ...
            for (key, ref_value) in ref_data {
                if !key.starts_with('_') {
                    ...
                    assert_eq!(
                        ...
                        entity_name, id, key, ref_value, turso_value
                    );
                }
            }
        }
...
fn generate_filter(_entity_names: Vec<String>, existing_values: Vec<String>) -> BoxedStrategy<Filter> {
    let leaf = prop_oneof![
        (Just("id".to_string()), prop::sample::select(existing_values.clone()))
            .prop_map(|(field, value)| Filter::Eq(field, Value::String(value))),
        (Just("value".to_string()), prop::sample::select(existing_values.clone()))
            .prop_map(|(field, value)| Filter::Eq(field, Value::String(value))),
        (Just("id".to_string()), prop::collection::vec(prop::sample::select(existing_values.clone()), 1..=3))
            .prop_map(|(field, values)| Filter::In(field, values.into_iter().map(Value::String).collect())),
    ...
    leaf.prop_recursive(
        ...
        |inner| {
            prop_oneof![
                ...
                prop::collection::vec(inner, 1..=2)
                    .prop_map(|filters| Filter::Or(filters)),
            ]
        },
    )
    .boxed()
...
fn generate_transitions(state: &ReferenceState) -> BoxedStrategy<StorageTransition> {
    ...
    let existing_ids: Vec<(String, String)> = state
        ...
        .iter()
        .flat_map(|(entity, ids)| {
            ids.keys().map(move |id| (entity.clone(), id.clone()))
        })
        .collect();
    ...
    if existing_ids.is_empty() {
        ...
        return prop::strategy::Union::new_weighted(vec![
            (10, create_entity),
            (40, insert),
        ])
        .boxed();
    ...
    let update = (
        prop::sample::select(existing_ids.clone()),
        "[a-z]{1,10}",
    )
        .prop_map(|((entity, id), value)| StorageTransition::Update {
            entity,
            id,
            value,
        })
    ...
    let set_version = (
        prop::sample::select(existing_ids),
        "[a-z0-9]{1,10}",
    )
...
impl StateMachineTest for StorageTest {
    ...
    fn apply(
        ...
        if let (Some(ref_entities), Some(turso_entities)) = (ref_result, turso_result) {
            ...
            ref_sorted.sort_by_key(|e| e.get("id").and_then(|v| v.as_string()).unwrap_or("").to_string());

            let mut turso_sorted = turso_entities.clone();
            turso_sorted.sort_by_key(|e| e.get("id").and_then(|v| v.as_string()).unwrap_or("").to_string());
    ...
    fn check_invariants(
        ...
        if ref_state.cdc_enabled {
            let conn = state.cdc_connection.as_ref()
            ...
            let expected_events: Vec<(String, String)> = ref_state.cdc_events.iter()
        ...
        for (view_name, actual_changes_arc) in &state.view_stream_changes {
            ...
            let expected_changes_arc = ref_state.view_stream_changes.get(view_name)
                .expect(&format!("View '{}' exists in SUT but not in reference state", view_name));
            ...
            // Compare each change
            for (i, (expected, actual)) in expected_changes.iter().zip(actual_changes.iter()).enumerate() {
                assert_eq!(
                ...
                assert_eq!(
                    expected_entity_id, actual_entity_id,
                    "\n\n=== View Change Entity ID Mismatch at index {} for '{}' ===\n\
                     ...
                     Actual change: {:?}\n\
                     =================================================\n",
                    i, view_name, expected_entity_id, actual_entity_id, expected.change, actual.change
                );

                // Compare change types
                match (&expected.change, &actual.change) {
                    (ChangeData::Created { data: exp_data, .. }, ChangeData::Created { data: act_data, .. }) => {
                    ...
                    (ChangeData::Updated { data: exp_data, .. }, ChangeData::Updated { data: act_data, .. }) => {
                        // Compare data excluding _rowid (internal implementation detail)
                        let mut exp_data_filtered = exp_data.clone();
                        let mut act_data_filtered = act_data.clone();
                        exp_data_filtered.remove("_rowid");
...
mod tests {
    ...
    async fn test_view_change_stream_receives_events_from_backend_operations() {
        ...
        backend.update("test_entity", "id1", update_data).await.unwrap();
