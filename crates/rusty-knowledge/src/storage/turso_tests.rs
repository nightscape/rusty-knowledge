...
mod value_conversion_tests {
    ...
    async fn test_value_to_sql_param_datetime() {
        let backend = create_test_backend().await;
        let dt = Utc::now();
        let result = backend.value_to_sql_param(&Value::DateTime(dt));
    ...
    async fn test_value_to_sql_param_json() {
        ...
        let result = backend.value_to_sql_param(&Value::Json(json));
...
mod view_cdc_tests {
    ...
    async fn test_view_cdc_basic_view_creation() {
        ...
        conn.execute(
            "CREATE VIEW basic_view AS SELECT * FROM view_base",
            ()
        ).await.unwrap();

        let mut rows = conn.query("SELECT COUNT(*) FROM basic_view", ()).await.unwrap();
    ...
    async fn test_view_cdc_filtered_view_with_trigger() {
        ...
        for i in 1..=5 {
            ...
            entity.insert("category".to_string(), Value::String(
                if i <= 3 { "bugs" } else { "features" }.to_string()
            ));
        ...
        let mut rows = conn.query("SELECT COUNT(*) FROM high_priority_bugs", ()).await.unwrap();
        if let Some(row) = rows.next().await.unwrap() {
            let count = row.get_value(0).unwrap();
            assert_eq!(count, turso::Value::Integer(1));
        }
        ...
        backend.update("view_filter_base", "item-2", updates).await.unwrap();
        let mut rows = conn.query("SELECT COUNT(*) FROM high_priority_bugs", ()).await.unwrap();
        if let Some(row) = rows.next().await.unwrap() {
            let count = row.get_value(0).unwrap();
