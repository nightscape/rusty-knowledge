...
//!
//! System-specific implementations should be in separate crates (e.g., rusty-knowledge-todoist).
...
///
/// ```ignore
/// use rusty_knowledge::sync::ExternalSystem;
/// use async_trait::async_trait;
///
...
pub trait ExternalSystem: Send + Sync {
    ...
    ///
    /// # Arguments
    /// - `command_type`: Operation type (e.g., "indent_block", "create_task")
...
/// Helper to convert serde_json::Value to rusty_knowledge Value enum
///
...
pub fn json_to_value(v: &serde_json::Value) -> Result<Value> {
    match v {
        ...
        serde_json::Value::Array(_) | serde_json::Value::Object(_) => Ok(Value::Json(v.clone())),
    }
}
