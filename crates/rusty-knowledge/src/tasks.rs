...
impl crate::core::Blocklike for Task {
    fn to_block(&self) -> crate::core::Block {
        crate::core::Block {
            id: self.id.clone(),
    ...
    fn from_block(block: &crate::core::Block) -> Option<Self> {
        if block.source != "internal" {
            return None;
        }

        Some(Self {
            id: block.source_id.clone(),
...
mod tests {
    ...
    use crate::core::traits::{HasSchema, Lens, Predicate};

    #[test]
    fn test_task_lenses_generated() {
        let task = Task::new("Test Task".to_string(), None);

        let id_lens = IdLens;
        assert_eq!(id_lens.get(&task), Some(task.id.clone()));

        let title_lens = TitleLens;
        assert_eq!(title_lens.get(&task), Some("Test Task".to_string()));

        let completed_lens = CompletedLens;
        assert_eq!(completed_lens.get(&task), Some(false));

        let parent_id_lens = ParentIdLens;
        assert_eq!(parent_id_lens.get(&task), None);
    }

    #[test]
    fn test_task_lens_set() {
        let mut task = Task::new("Original".to_string(), None);

        let title_lens = TitleLens;
        title_lens.set(&mut task, "Updated".to_string());
        assert_eq!(task.title, "Updated");

        let completed_lens = CompletedLens;
        completed_lens.set(&mut task, true);
    ...
    fn test_task_to_entity() {
        ...
        assert_eq!(entity.get("title").unwrap().as_str().unwrap(), "Convert Me");
        assert!(!entity.get("completed").unwrap().as_bool().unwrap());
        assert_eq!(
            entity.get("parent_id").unwrap().as_str().unwrap(),
            "parent-123"
        );
    }
    ...
    fn test_task_from_entity() {
        ...
        let mut entity = Entity::new("tasks");
    ...
    #[test]
    fn test_task_predicate_with_lens() {
        let task1 = Task::new("Incomplete".to_string(), None);
        let mut task2 = Task::new("Complete".to_string(), None);
        task2.completed = true;

        let completed_lens = CompletedLens;
        let is_completed = Eq::new(completed_lens, true);

        assert!(!is_completed.test(&task1));
        assert!(is_completed.test(&task2));
    }
}
