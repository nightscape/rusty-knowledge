...
use crate::codegen::parser::mir::parser::ty::generics::{
    is_generic_template, parse_generics_info, GenericsInfo,
};
...
fn parse_item<Item: SynItemStructOrEnum>(
    ...
    log::info!(
        "extra_type parsing item: {} (is_generic_template={})",
        item.name.name,
        is_generic_template(item.src.generics())
    );
    ...
    if is_generic_template(item.src.generics()) {
        log::info!(
            "Generic template {} parsed and stored, returning None for MirExtraType",
            item.name.name
        );
        return Ok(None);
    }
