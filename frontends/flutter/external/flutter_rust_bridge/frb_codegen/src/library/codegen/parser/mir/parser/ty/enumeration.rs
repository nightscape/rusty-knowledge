...
pub(crate) fn generate_type_name_for_auto_naming(ty: &MirType) -> String {
    match ty {
        ...
        MirType::Optional(opt) => format!(
            "Optional_{}",
            generate_type_name_for_auto_naming(&opt.inner)
        ),
        MirType::Boxed(boxed) => {
            format!("Box_{}", generate_type_name_for_auto_naming(&boxed.inner))
        }
        MirType::GeneralList(list) => {
            format!("List_{}", generate_type_name_for_auto_naming(&list.inner))
        }
        MirType::Delegate(delegate) => match delegate {
            ...
            MirTypeDelegate::StreamSink(sink) => {
                format!(
                    "StreamSink_{}",
                    generate_type_name_for_auto_naming(&sink.inner_ok)
                )
            }
...
impl TypeParserWithContext<'_, '_, '_> {
    ...
    pub(crate) fn parse_type_path_data_enum_with_alias(
        ...
        log::info!(
            "parse_type_path_data_enum_with_alias: name={}, generic_args.len()={}, alias_name={:?}",
            name,
            generic_args.len(),
            alias_name
        );
        ...
        if !generic_args.is_empty() {
            ...
            if !has_concrete_args {
                ...
                return EnumOrStructParserEnum(self).parse(
                    &template_path,
                    &(name, empty_args),
                    None,
                );
        ...
        if !generic_args.is_empty() {
            ...
            if let Some(namespace) = namespace.clone() {
                let template_name = crate::utils::namespace::NamespacedName::new(
                    namespace.clone(),
                    name.to_string(),
                );
                ...
                log::info!(
                    "Checking for generic template: {:?} with alias: {:?}",
                    template_ident,
                    alias_name
                );
                    ...
                    .contains_key(&template_ident)
                {
                    ...
                    let empty_args: &[syn::Type] = &[];
                    let _ = EnumOrStructParserEnum(self).parse(
                        &template_path,
                        &(name, empty_args),
                        None,
                    )?;
                    ...
                    .cloned()
                {
                    ...
                    let concrete_name = if let Some(alias) = alias_name {
                        ...
                        log::info!(
                            "Auto-generating name for nested generic enum: {} -> {}",
                            name,
                            auto_name
                        );
                        ...
                        .contains_key(&concrete_ident)
                    {
                        ...
                        return Ok(Some(MirType::EnumRef(
