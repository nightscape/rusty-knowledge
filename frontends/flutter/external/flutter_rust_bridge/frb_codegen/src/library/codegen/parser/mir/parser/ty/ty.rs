...
impl TypeParserWithContext<'_, '_, '_> {
    pub(crate) fn parse_type(&mut self, ty: &Type) -> anyhow::Result<MirType> {
        // Check if this is a type alias before resolving
        ...
        // If we're parsing a type alias, set the context flag for nested types
        ...
        let ans =
            parser_with_context.parse_type_inner_with_alias(&resolve_ty, alias_name.as_deref())?;
        let ty_str = ty_to_string(ty);
        if ty_str.contains("Change") {
            log::info!(
                "TypeParserWithContext.parse_type ty={} resolved_ty={} alias_name={:?} ans={ans:?}",
                ty_str,
                ty_to_string(&resolve_ty),
                alias_name
            );
        }
        log::debug!("TypeParserWithContext.parse_type ty={} ans={ans:?}", ty_str);
        Ok(ans)
    }

    fn parse_type_inner_with_alias(
        &mut self,
        ty: &Type,
        alias_name: Option<&str>,
    ) -> anyhow::Result<MirType> {
        // Store alias name in context for use during parsing
        // This will be used when instantiating generic templates
        let ans = match ty.clone() {
            Type::Path(x) => {
                // Pass alias name to path parsing
                self.parse_type_path_with_alias(&x, alias_name)?
            }
    ...
    fn parse_type_path_with_alias(
        ...
        match &type_path {
            syn::TypePath { qself: None, path } => {
                self.parse_type_path_core_with_alias(type_path, path, alias_name)
            }
            syn::TypePath {
    ...
    fn parse_type_path_core_with_alias(
        &mut self,
        type_path: &syn::TypePath,
        path: &syn::Path,
        alias_name: Option<&str>,
        ...
        if let Some(last_segment) = splayed_segments.last() {
            ...
            if let Some(ans) =
                self.parse_type_path_data_struct_with_alias(path, last_segment, None, alias_name)?
            {
                return Ok(ans);
            }
            // Pass alias_name to enum parsing
            if let Some(ans) =
                self.parse_type_path_data_enum_with_alias(path, last_segment, alias_name)?
            {
                return Ok(ans);
            }
